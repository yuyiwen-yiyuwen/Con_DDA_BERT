...
def discover_pkl_files(pkl_dir: str) -> list[str]:
...
class PKLBatchIterableDataset(IterableDataset):
    def __init__(
        ...
        super().__init__()
        all_files = sorted(pkl_files) if pkl_files is not None else discover_pkl_files(pkl_dir)
    ...
    def __iter__(self):
        ...
        for fp in files:
            with _open_maybe_gz(fp, "rb") as f:
                data = pickle.load(f)
...
def parse_args():
    parser = argparse.ArgumentParser(description="MSGPT 训练脚本（pkl.gz 支持）")
    parser.add_argument("--config", type=str, default="/home/yiwen/AIPC/scripts/organized_attantion/config/model.yaml", help="配置文件路径")
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default="/home/yiwen/AIPC/scripts/organized_attantion/data/pkl_dataset_tims_select/train",
        help="训练数据目录，支持 pkl 或 pkl.gz",
    )
    parser.add_argument("--num_workers", type=int, default=4, help="DataLoader worker 数")
    parser.add_argument(
        "--model_save_path",
        type=str,
        default="/home/yiwen/AIPC/scripts/organized_attantion/checkpoints",
        help="模型保存目录",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="/home/yiwen/AIPC/scripts/organized_attantion/data/pkl_dataset_tims_select/train",
        help="在线重排模式的输入目录（原始 pkl/pkl.gz）",
    )
    parser.add_argument(
...
def main():
    ...
    pkl_files = discover_pkl_files(selected_train_dir)
    print(f"发现训练文件: {len(pkl_files)} 个 (.pkl + .pkl.gz)")
    ...
    print(
        f"读取已编码训练文件数(pkl/pkl.gz): 总计={len(pkl_files)}"
    )
    print(
