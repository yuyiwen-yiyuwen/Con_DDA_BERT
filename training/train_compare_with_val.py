...
class OnlineMassAnchoredBatchIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.extra_pkl_files = list(extra_pkl_files) if extra_pkl_files else []
        self.epoch = 0
        self.payloads = []
        self.lengths = []
        self.batch_ranges = []
    ...
    def _prepare_global_order(self):
        ...
        # 额外加入 val 等数据集的 pkl 文件
        if self.extra_pkl_files:
            files = sorted(files + self.extra_pkl_files)
            # 去重
            seen = set()
...
def parse_args():
    parser = argparse.ArgumentParser(description="MSGPT 本地训练脚本（加速版，含 val 数据）")
    ...
    parser.add_argument(
        ...
        help="额外加入的验证数据目录（排除 val.00004_val.pkl）",
    )
    parser.add_argument("--num_workers", type=int, default=4, help="DataLoader worker 数")
    parser.add_argument(
        "--model_save_path",
        type=str,
        default="/home/yiwen/AIPC/scripts/organized_attantion/data/checkpoints_withoutRT_with_val",
        help="模型保存目录",
    )
...
def _gather_val_pkl_files(val_data_dir: str) -> list[str]:
...
def main():
    ...
    # 收集 val 目录下的 pkl 文件（排除 val.00004_val.pkl）
    val_pkl_files = _gather_val_pkl_files(args.val_data_dir)
    ...
    # 查找训练数据地址中所有pkl文件，并合并 val pkl 文件
    # glob库用于批量查找文件，os库用于各种文件、路径和系统操作
    # pkl_files：文件地址列表
    pkl_files = sorted(glob.glob(os.path.join(selected_train_dir, "*.pkl")))
    # 合并 train + val 的 pkl 文件
    all_pkl_files = sorted(pkl_files + val_pkl_files)
    if not all_pkl_files:
        raise ValueError(
            f"在 {selected_train_dir} 和 {args.val_data_dir} 下均未发现 pkl 文件。"
            "当前训练入口只实现了 pkl 流式读取，请先转换数据或切换到 parquet 训练脚本。"
        )

    print(f"训练 pkl 文件: train={len(pkl_files)}, val={len(val_pkl_files)}, 合计={len(all_pkl_files)}")
    ...
    print(
        f"读取已编码训练文件数(pkl): 总计={len(all_pkl_files)}"
    )
    print(
