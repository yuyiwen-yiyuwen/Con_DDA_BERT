...
def _start_heartbeat(state: dict, stop_event: threading.Event, tag: str = "心跳") -> threading.Thread:
    """每隔固定秒数输出一次当前阶段状态，避免长耗时步骤无日志。"""
    ...
    def _run():
        while not stop_event.wait(PROGRESS_REFRESH_SEC):
            ...
            tqdm.write(
                f"[{tag}] {idx}/{total} | stage={stage} | file={current_file}"
            )
...
def _build_mass_anchored_order(
    ...
    with tqdm(
        ...
        unit="sample",
        dynamic_ncols=True,
        mininterval=PROGRESS_REFRESH_SEC,
    ) as pbar:
        built_batches = 0
        while cursor < n:
            ...
            built_batches += 1
            pbar.set_postfix(
                batches=f"{built_batches}/{total_batches}",
                anchor_mass=f"{anchor_mass:.2f}",
                window_da=f"{window:.2f}",
                refresh=False,
            )
            cursor = right
...
def _enhance_batch_peptide_diversity(
    ...
    for bidx, s in enumerate(
        ...
        for p in dup_pos:
            found = False
            while k < k_end:
                cand_pep = peptide_ids[order[k]]
                if cand_pep not in unique_in_batch:
        ...
        if bidx % 10 == 0:
            tqdm.write(
                f"去重增强进度: batch {bidx}/{total_batches}, "
                f"dup={len(dup_pos)}, swapped={swap_cnt}"
            )
...
class OnlineMassAnchoredBatchIterableDataset(IterableDataset):
    """在线质量锚定重排：一次性计算全局顺序，训练时逐批直接产出，不落盘。"""
    ...
    def _prepare_global_order(self):
        ...
        if not files:
            raise FileNotFoundError(f"在 {self.input_dir} 下未发现 pkl 文件")

        all_masses = []
        all_peptide_ids = []

        print(f"在线重排: 扫描文件数={len(files)}")
        print(f"在线重排: 读取阶段已开始，若单文件较慢将每 {PROGRESS_REFRESH_SEC:.0f}s 输出一次心跳")
        total_files = len(files)
        heartbeat_state = {"idx": 0, "total": total_files, "stage": "init", "file": "-"}
        heartbeat_stop = threading.Event()
        heartbeat_thread = _start_heartbeat(heartbeat_state, heartbeat_stop, tag="读取进度")
        ...
        try:
            for i, fp in enumerate(
                ...
                heartbeat_state["stage"] = "extract_peptide_ids"
                ...
                pep_ids = _extract_peptide_ids(payload, n)

                heartbeat_state["stage"] = "append_arrays"

                self.payloads.append(payload)
                self.lengths.append(n)
                all_masses.append(np.asarray(precursors[:, 0], dtype=np.float64))
                all_peptide_ids.append(pep_ids)

                if (i % 20) == 0 or i == total_files:
                    tqdm.write(
                        f"读取进度: {i}/{total_files} files, "
                        f"累计样本={int(np.sum(self.lengths))}"
                    )
        finally:
            heartbeat_stop.set()
        ...
        total_samples = int(masses.shape[0])
        print(f"在线重排: 总样本数={total_samples}")

        rng = np.random.default_rng(self.seed)
        global_order = _build_mass_anchored_order(
            masses=masses,
            batch_size=self.batch_size,
            start_window_da=self.start_window_da,
            expand_factor=self.expand_factor,
            max_window_da=self.max_window_da,
            rng=rng,
        )
        ...
        print(f"在线重排: batch 数={len(self.batch_ranges)} (drop_last={self.drop_last})")
    ...
    def __iter__(self):
        ...
        if worker_info is not None and worker_info.num_workers > 1:
            raise RuntimeError("在线重排模式不支持 num_workers>0，请将 --num_workers 设为 0")
        ...
        for bidx in batch_indices:
            ...
            if "weight" in self.payloads[0]:
                weights = self._gather_ndarray("weight", seg_file_ids, seg_local_ids)
            else:
                weights = np.ones_like(labels)

            if "unmask" in self.payloads[0]:
                unmask = self._gather_ndarray("unmask", seg_file_ids, seg_local_ids)
            else:
                unmask = np.zeros_like(labels)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="学习率调度使用的总轮次，0 表示使用配置文件里的 epochs",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="每个 pkl 文件随机抽样比例，范围 (0, 1]，默认 1.0（全量训练）",
    )
    parser.add_argument(
        "--shuffle_within_file",
        action="store_true",
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="/home/yiwen/AIPC/scripts/attantion/checkpoints_compare/msgpt_epoch_7.pt",
        help="启动时加载的权重路径",
    )
    parser.add_argument(
        "--resume_epoch",
        type=int,
        default=-1,
        help="恢复训练的起始 epoch（0-based）。-1 表示优先使用 checkpoint 里的 epoch",
    )
    parser.add_argument(
        "--resume_global_step",
        type=int,
        default=-1,
        help="恢复训练的全局步数。-1 表示自动按 start_epoch*steps_per_epoch 重算",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prefer_checkpoint_global_step",
        action="store_true",
        help="未显式传 resume_global_step 时优先采用 checkpoint 里的 global_step",
    )
...
def main():
    ...
    if args.resume_epoch < -1:
        raise ValueError("--resume_epoch 不能小于 -1")
    if args.resume_global_step < -1:
        raise ValueError("--resume_global_step 不能小于 -1")
    if args.lr_schedule_total_epochs < 0:
        raise ValueError("--lr_schedule_total_epochs 不能小于 0")
    if args.online_start_window_da <= 0:
        raise ValueError("--online_start_window_da 必须大于 0")
    if args.online_expand_factor <= 1.0:
        raise ValueError("--online_expand_factor 必须大于 1")
    if args.online_max_window_da <= 0:
        raise ValueError("--online_max_window_da 必须大于 0")
    ...
    if args.online_mass_anchor_train:
        if not os.path.isdir(args.online_input_dir):
            raise FileNotFoundError(f"找不到在线重排输入目录: {args.online_input_dir}")
    else:
    ...
    if args.online_mass_anchor_train:
        ...
        print(f"在线重排模式 steps_per_epoch={steps_per_epoch}")
    else:
        # 查找训练数据地址中所有pkl文件
    ...
    # 训练执行轮次：命令行 --epochs > 0 时覆盖配置文件。
    config_total_epochs = int(config.get("epochs", 25))
    epochs = int(args.epochs) if int(args.epochs) > 0 else config_total_epochs
    # 学习率调度轮次：默认沿用配置文件总轮次，不随本次只训练到某个 epoch 而压缩。
    lr_schedule_total_epochs = (
        int(args.lr_schedule_total_epochs)
        if int(args.lr_schedule_total_epochs) > 0
        else config_total_epochs
    )

    # 估计 its
    if (not args.online_mass_anchor_train) and use_pkl_stream:
    ...
    elif not args.online_mass_anchor_train:
        steps_per_epoch = len(train_loader)

    # 训练恢复信息：按“从 0 训到目标 epoch”的时间轴衔接调度器。
    ...
    if start_epoch < 0 or start_epoch > epochs:
        raise ValueError(f"resume_epoch 非法: {start_epoch}, 期望范围 [0, {epochs}]")
    if lr_schedule_total_epochs < (start_epoch + 1):
        raise ValueError(
            f"lr_schedule_total_epochs={lr_schedule_total_epochs} 过小，"
            f"至少应 >= start_epoch+1={start_epoch + 1}"
        )

    if (not args.online_mass_anchor_train) and start_epoch > 0 and float(args.sample_ratio) < 1.0:
        print(
            "警告: 当前为恢复训练且 sample_ratio<1.0，steps_per_epoch 会变小，"
            "学习率时间轴将不再严格等价于从 0 连续训练到目标 epoch。"
        )

    estimated_start_global_step = max(start_epoch * steps_per_epoch, 0)
    if args.resume_global_step >= 0:
        global_step = int(args.resume_global_step)
    elif bool(args.prefer_checkpoint_global_step) and (ckpt_global_step is not None):
        global_step = int(ckpt_global_step)
    else:
        global_step = estimated_start_global_step

    if (ckpt_global_step is not None) and (global_step != int(ckpt_global_step)):
        print(
            f"提示: 已忽略 checkpoint.global_step={int(ckpt_global_step)}，"
            f"改用重算后的 start_global_step={global_step} 以对齐当前时间轴。"
        )

    scheduler_last_batch_iteration = max(global_step - 1, -1)
    print(
        f"恢复训练: start_epoch={start_epoch}, epochs={epochs}, "
        f"start_global_step={global_step}, steps_per_epoch={steps_per_epoch}"
    )

    # 学习率时间轴按“调度总轮次”计算，不受本次训练提前结束影响。
    # 这样从 checkpoint 恢复时，lr 会对齐到“正常训练进行到当前轮次”的位置。
    scheduled_total_steps = lr_schedule_total_epochs * steps_per_epoch
    ...
    scheduler = WarmupScheduler(
        ...
        last_batch_iteration=scheduler_last_batch_iteration,
    )
    # 恢复训练时立即把 optimizer.lr 同步到当前 global_step 对应位置。
    scheduler.step(scheduler_last_batch_iteration)
    print(
        "LR调度信息: "
        f"train_epochs={epochs}, "
        f"schedule_total_epochs={lr_schedule_total_epochs}, "
        f"steps_per_epoch={steps_per_epoch}, "
        f"scheduled_total_steps={max_iters}, "
        f"warmup_iters={warmup_iters}, "
        f"start_iter={scheduler_last_batch_iteration}, "
        f"start_lr={scheduler.get_last_lr()[0]:.2e}"
    )
