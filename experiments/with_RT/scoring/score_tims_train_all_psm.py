...
def parse_args():
    parser = argparse.ArgumentParser(description="Score all PSM rows in tims PKL files with MSGPT.")
    parser.add_argument(
        "--model_path",
        type=str,
...
def score_one_pkl(pkl_path, out_dir, model, device, batch_size, num_workers, overwrite, s2i, n_peaks):
    ...
    # 这里的 pkl.gz 实际上是一个 dict，包含 'label' 等 key
...
def main():
    ...
    if len(train_files) == 0 and len(val_files) == 0:
        raise RuntimeError(
            ...
            "示例 train: /home/yiwen/AIPC/scripts/attantion/pkl_dataset_tims/tims_pkl_all/train"
        )
