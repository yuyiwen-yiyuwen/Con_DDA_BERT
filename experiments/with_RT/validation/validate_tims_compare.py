...
from model.transformer_compare_rt.model import MSGPT
...
def parse_args():
    ...
    parser.add_argument(
        "--val_data_dir",
        type=str,
        default="/home/yiwen/AIPC/scripts/attantion/pkl_dataset/val_select",
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="/home/yiwen/AIPC/scripts/attantion/checkpoints_tims",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.002,
        help="Random sampling ratio per pkl file in (0, 1]. Default: 0.002",
    )

    parser.add_argument(
        "--results_dir",
        type=str,
        default="/home/yiwen/AIPC/scripts/attantion/val_train_results_tims",
...
def build_random_subset_indices_by_file(file_offsets, file_lengths, sample_ratio):
    if not (0 < sample_ratio <= 1):
        raise ValueError(f"file_sample_ratio must be in (0, 1], got {sample_ratio}")

    rng = np.random.default_rng()
    subset_indices = []

    for offset, file_len in zip(file_offsets, file_lengths):
        if file_len <= 0:
            continue

        sample_count = int(np.floor(file_len * sample_ratio))
        sample_count = max(1, sample_count)
        sample_count = min(sample_count, file_len)

        selected_rel = rng.choice(file_len, size=sample_count, replace=False)
        subset_indices.extend((offset + selected_rel).tolist())
...
def infer_vocab_size(config: dict):
...
def attach_no_mlm_head(model):
    ...
    # 替换为无参数占位层，确保 state_dict 严格匹配无 mask_lm 的 checkpoint。
...
def evaluate_one_checkpoint(ckpt_path, base_config, loader, device):
    ...
    try:
        ...
        model_config = ckpt_config if isinstance(ckpt_config, dict) and ckpt_config else base_config
        model = build_model(model_config, vocab_size=infer_vocab_size(model_config))
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"Validating {model_name}", leave=False):
            ...
            _, _, dda_logits, _ = model(spectra, spectra_mask, precursors, tokens)
...
def main():
    ...
    subset_indices = build_random_subset_indices_by_file(
        file_offsets=full_dataset.file_offsets,
        file_lengths=full_dataset.file_lengths,
        sample_ratio=args.file_sample_ratio,
    )

    if not subset_indices:
        print(
            f"No samples selected with file_sample_ratio={args.file_sample_ratio}. "
    ...
    print(
        f"Using {num_samples} samples for validation "
        f"(file_sample_ratio={args.file_sample_ratio})."
    )
    ...
    for ckpt_path in ckpt_files_to_eval:
        ...
        result, loss_info = evaluate_one_checkpoint(ckpt_path, config, loader, device)
